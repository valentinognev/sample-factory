...
class PointTrajectoryEnv(gym.Env, TrainingInfoInterface):
    ...
    def __init__(self, full_env_name: str, cfg=None, env_config=None, render_mode=None):
        ...
        self.action_space = spaces.Box(
            low=np.array([-self.max_speed, -self.max_speed, -30.0], dtype=np.float32),
            high=np.array([ self.max_speed,  self.max_speed,  30.0], dtype=np.float32),
            dtype=np.float32
        )
        obs_low  = np.array([0.0, -np.pi, -self.max_speed], dtype=np.float32)
        obs_high = np.array([20.0,  np.pi,  self.max_speed], dtype=np.float32)
        ...
        # -- CSV LOGGING SETUP ------------------------------------------
        # create a logs folder (matches enjoy?s log_dir)
        ...
        self._csv_writer.writerow([
            "time","agent_N","agent_E",
            "target_N","target_E","r",
            "theta","radial_rate","v_f_cmd",
            "v_r_cmd","w_cmd","vf","vr","w",
            "dE","dN","reward"
        ])
    ...
    def _generate_trajectory(self, traj_type: str):
        ...
        if traj_type == 'circle':
            ...
            # self.trajectory_points = np.array([[2.0, 1.0]] * N, dtype=np.float32)  # fixed-point target for debugging
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.last_r = r
        obs = np.array([r, theta, 0.0], dtype=np.float32)
        self.step_count = 0
    ...
    def step(self, action):
        ...
        self.w  = beta * self.w + (1.0 - beta) * w_cmd
        ...
        r, theta = self._get_obs()
        self.theta = theta ## DEBUG
        radial_rate = (old_r - r)/self.dt
        self.last_r = r
        obs = np.array([r, theta, radial_rate], dtype=np.float32)

        # Reward
        dist_part  = -self.r_reward*(r**2)
        angle_part = -(self.theta_reward * (np.sqrt(np.abs(theta))) * # degrade angle component when too close to target
                       np.clip((np.abs(r) - self.reward_min) / (self.reward_max - self.reward_min), 0.0, 1.0)) 
        action_penalty = -self.AP_factor*(np.sum(abs(action)))
        reward     = dist_part + angle_part + 0.0*action_penalty
        ...
        self._csv_writer.writerow([
            ...
            r, theta, radial_rate,
            v_f_cmd,      # radial velocity ? ?v_f?
            v_r_cmd,      # tangential velocity ? ?v_r?
            w_cmd,        # yaw rate
            self.vf,
            self.vr,
            self.w,
            dE,       # velocity on East-axis
            dN,       # velocity on North-axis
            reward
        ])
        self._log_file.flush()
        ...
        info = {"r":float(r), "theta":float(theta), "radial_rate":float(radial_rate)}
    ...
    def render(self, mode='human'):
        ...
        self.ax_top.text(-10.0,-10.0,str(self.vf_cmd))
        self.ax_top.text(-10.0,-12.0,str(self.vr_cmd))
        self.ax_top.text(-10.0,-14.0,str(self.w_cmd))
