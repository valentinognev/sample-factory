...
class PointTrajectoryEnv(gym.Env, TrainingInfoInterface):
    ...
    def __init__(self, full_env_name: str, cfg=None, env_config=None, render_mode=None):
        ...
        obs_low  = np.array([0.0, -np.pi, -self.max_speed, -np.pi], dtype=np.float32)
        obs_high = np.array([20.0,  np.pi,  self.max_speed, np.pi], dtype=np.float32)
    ...
    def step(self, action):
        ...
        reward = dist_part + 0.0*angle_part + 0.0*action_penalty

        # Accumulate per-episode reward components for TensorBoard (training picks these up via 'episode_extra_stats')
